...
const elements = {
    ...
    keepOnlySession: document.querySelector("#keep-only-session"),
    ...
    drLatSec: document.querySelector("#dr-lat-sec"),
    ...
    drLonSec: document.querySelector("#dr-lon-sec"),
    vesselMotionEnabled: document.querySelector("#vessel-motion-enabled"),
    motionReferenceTime: document.querySelector("#motion-reference-time"),
    motionCourse: document.querySelector("#motion-course"),
...
function updateMotionAvailability() {
    const enabled = Boolean(elements.vesselMotionEnabled?.checked);
    const fields = [
        elements.motionReferenceTime,
        elements.motionCourse,
...
function coordinatePartsToDecimal(direction, degreesElement, minutesElement, secondsElement, maxDegrees) {
    const degrees = getNumberFromElement(degreesElement);
    const minutes = getNumberFromElement(minutesElement);
    const seconds = getNumberFromElement(secondsElement);

    if (degrees < 0 || degrees > maxDegrees || minutes < 0 || minutes >= 60 || seconds < 0 || seconds >= 60) {
        throw new Error(`Nieprawidłowy format pozycji DR. Minuty i sekundy muszą być w zakresie 0-59,999, a stopnie nie mogą przekraczać ${maxDegrees}.`);
    }
...
function decimalToCoordinateParts(value, type) {
    ...
    return {
        ...
        degrees,
        minutes,
        seconds: decimalComma(seconds.toFixed(2)),
    };
}
    ...
    function getDeadReckoningData() {
        return {
            ...
            motion: {
                ...
                referenceTime: getInputValue("#motion-reference-time"),
                course: getNumberValue("#motion-course"),
                speed: getNumberValue("#motion-speed"),
            },
        };
    }
    ...
    function setDeadReckoningData(data) {
        ...
        if (elements.drLatSec) {
            elements.drLatSec.value = latitude.seconds;
        }
        ...
        if (elements.drLonSec) {
            elements.drLonSec.value = longitude.seconds;
        }
        ...
        if (elements.vesselMotionEnabled) {
            elements.vesselMotionEnabled.checked = Boolean(motion.enabled);
        }
        setInputValue("#motion-reference-time", motion.referenceTime);
        setInputValue("#motion-course", motion.course);
        setInputValue("#motion-speed", motion.speed);
...
if (body === "Venus" || body === "Mars") {
  ...
  if (body === "Mars") {
    ...
    function parseAngleToParts(value) {
        const text = String(value ?? "").trim();
        ...
        const cleaned = text
            ...
            .replace(/\s+/g, "")
            .replace(/:+$/g, "");
        const [degrees = "", minutes = "", seconds = ""] = cleaned.split(":");

        return {
            degrees: normalizeDecimalDisplay(degrees),
            minutes: normalizeDecimalDisplay(minutes),
            seconds: normalizeDecimalDisplay(seconds),
        };
    }

    function setAngleFields(prefix, value, options = {}) {
        const { seconds = false } = options;
        const parts = parseAngleToParts(value);

        setInputValue(`#${prefix}-deg`, parts.degrees);
        setInputValue(`#${prefix}-min`, parts.minutes);

        if (seconds) {
            setInputValue(`#${prefix}-sec`, parts.seconds);
        }
    }

    function readAngleNumber(value) {
        const number = Number(String(value ?? "").trim().replace(/,/g, "."));
        return Number.isFinite(number) ? number : null;
    }

    function readAngleFromFields(prefix, options = {}) {
        const { seconds = false, required = true, label = "kąta" } = options;
        const degrees = getInputValue(`#${prefix}-deg`);
        const minutes = getInputValue(`#${prefix}-min`);
        const angleSeconds = seconds ? getInputValue(`#${prefix}-sec`) : "";
        const hasAnyValue = Boolean(degrees || minutes || angleSeconds);

        if (!required && !hasAnyValue) {
            return "";
        }

        if (!degrees || !minutes || (seconds && !angleSeconds)) {
            throw new Error(`Uzupełnij wszystkie pola dla ${label}.`);
        }

        const degreesNumber = readAngleNumber(degrees);
        const minutesNumber = readAngleNumber(minutes);
        const secondsNumber = seconds ? readAngleNumber(angleSeconds) : 0;

        if (degreesNumber === null || minutesNumber === null || secondsNumber === null) {
            throw new Error(`Nieprawidłowa wartość liczbowa dla ${label}.`);
        }

        if (minutesNumber < 0 || minutesNumber >= 60 || secondsNumber < 0 || secondsNumber >= 60) {
            throw new Error(`Minuty i sekundy dla ${label} muszą być w zakresie 0-59,999.`);
        }

        const formattedDegrees = normalizeDecimalDisplay(degrees);
        const formattedMinutes = normalizeDecimalDisplay(minutes);
        const formattedSeconds = normalizeDecimalDisplay(angleSeconds);
    ...
    function applyAlmanacValues(values) {
        setAngleFields("gha0", values.gha0);
        setAngleFields("gha1", values.gha1);
        setAngleFields("dec0", values.dec0);
        setAngleFields("dec1", values.dec1);
        setAngleFields("sha", values.sha);
    ...
    function getObservationFormData() {
        ...
        return {
            ...
            hs: readAngleFromFields("hs", { seconds: true, label: "wysokości sekstantowej Hs" }),
            gha0: readAngleFromFields("gha0", { label: "GHA 0h" }),
            gha1: readAngleFromFields("gha1", { label: "GHA 1h" }),
            dec0: readAngleFromFields("dec0", { label: "deklinacji 0h" }),
            dec1: readAngleFromFields("dec1", { label: "deklinacji 1h" }),
            sha: readAngleFromFields("sha", { required: false, label: "SHA" }),
            limb: getInputValue("#limb"),
            sd: getInputValue("#sd"),
            hp: getInputValue("#hp"),
            artificialHorizon: Boolean(document.querySelector("#artificial-horizon")?.checked),
            correctedAltitude: Boolean(document.querySelector("#corrected-altitude")?.checked),
            computed:
            {
                ho: null,
                hc: null,
  ...
  const rowsHtml = observations.map((observation, index) => `
    <tr>
      ...
      <td>${formatAngleForInput(observation.hs) || "-"}</td>
...
function handleDeleteSession() {
    ...
    if (!session) {
        alert("Wybierz sesję do usunięcia.");
        return;
    }

    if (!confirm(`Usunąć sesję "${session.name}"?`)) {
        return;
    }
...
function handleKeepOnlySession() {
    const session = getSelectedSession();

    if (!session) {
        alert("Wybierz sesję, którą chcesz zostawić.");
        return;
    }

    if (!confirm(`Zostawić tylko sesję "${session.name}" i usunąć pozostałe?`)) {
        return;
    }
    ...
    if (elements.savedSessions) {
        elements.savedSessions.value = session.id;
    }

    alert("Zostawiono tylko wybraną sesję.");
}
...
function bindEvents() {
    ...
    elements.vesselMotionEnabled?.addEventListener("change", updateMotionAvailability);
    ...
    elements.deleteSession?.addEventListener("click", handleDeleteSession);
    elements.keepOnlySession?.addEventListener("click", handleKeepOnlySession);
}
function init() {
    setCurrentUtcDateTime();
    updateLimbAvailability();
